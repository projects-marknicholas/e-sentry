...
/**
 * BigPlotlyCharts component displays a Plotly line chart with data, prediction, upper and lower bounds.
 *
...
const BigPlotlyCharts = ({
  ...
  const layout = {
    title: `${parameter} filtered by ${filterOption}`,
    xaxis: {
      title: "Date",
      range: [
        new Date(Math.min(...filteredData)) - 24 * 60 * 60 * 1000,
        new Date(Math.max(...filteredData)) + 24 * 60 * 60 * 1000,
      ],
    },
    yaxis: { title: "Value" },
    width: 1140,
